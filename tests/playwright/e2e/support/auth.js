...
async function login(page, username, password) {
  ...
  for (let attempt = 1; attempt <= 3; attempt++) {
    ...
    // Let every subresource finish before submitting. Moodle stores the one-shot login token in
    // the session, and on a cold cache the page's own CSS and JavaScript are generated by PHP
    // requests that load and save that same session. A form submitted while one of them is still
    // in flight has its token overwritten when that request writes the session back - and Moodle
    // reports the mismatch as "Invalid login", with correct credentials. It showed up reliably
    // right after a cache purge, which is exactly when those requests are slow.
