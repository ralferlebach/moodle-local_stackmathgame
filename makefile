# Makefile for local_stackmathgame
...
#
# local_stackmathgame bundles three question-game mode subplugins under mode/. They are
# separate Moodle components with their own version.php and their own amd/src, so the AMD
# and ESLint targets iterate over them explicitly - a run scoped to the parent plugin does
# not see them.
#
# Paths are auto-detected from the makefile's own location.
# The plugin lives at <MOODLE_ROOT>/local/stackmathgame/ —
...
PLUGIN_NAME   ?= local_stackmathgame
PLUGIN_REL    ?= local/stackmathgame
...
#   make jmeter BASE_URL=http://localhost/moodle45_aliseadele TOKEN=... QUIZID=.. CMID=..
BASE_URL       ?= $(or $(MOODLE_WWWROOT),http://localhost/moodle45_aliseadele)
TOKEN          ?=
QUIZID    ?=
CMID           ?=
REVISION       ?= 2000
...
# Values written by `make load-seed` (BASE_URL/TOKEN/QUIZID/CMID). Auto-read
# ignores the file when it does not exist yet; a command-line override wins.
-include $(LOAD_DIR)/.load-env
# Optional: override the base URL Playwright uses (otherwise seed.php sets it from
# $CFG->wwwroot).
SMG_BASE_URL ?= $(MOODLE_WWWROOT)
...
lint-js:
	...
	@# Same reason as in the amd target: the mode subplugins are separate components and are
	@# not reached by a lint run scoped to the parent plugin's amd/src.
	@for mode in $(PLUGIN_DIR)/mode/*/; do \
		if ls $$mode/amd/src/*.js >/dev/null 2>&1; then \
			echo "  ESLint: $$(basename $$mode)"; \
			(cd $(MOODLE_ROOT) && $(NPX) grunt eslint --root=. \
				--files=$(PLUGIN_REL)/mode/$$(basename $$mode)/amd/src/ \
				--show-lint-warnings) || true; \
		fi; \
	done
...
amd:
	...
	@# The three bundled mode subplugins each own an amd/src/game.js. Grunt resolves the
	@# component from the directory it runs in, so a single run in the plugin root builds
	@# amd/build/ but leaves mode/*/amd/build/ stale — which then ships an outdated game
	@# module while amd/src looks perfectly up to date.
	@for mode in $(PLUGIN_DIR)/mode/*/; do \
		if ls $$mode/amd/src/*.js >/dev/null 2>&1; then \
			echo "  AMD: $$(basename $$mode)"; \
			(cd $$mode && $(NPX) grunt amd --force) || true; \
		fi; \
	done
...
# Need a RUNNING Moodle site with local_stackmathgame installed. seed.php creates the
# course-mode fixture and prints the exports (incl. SMG_BASE_URL from the
...
playwright: clear
	...
	cd $(PLAYWRIGHT_DIR) && eval "$$($(PHP) seed.php)" && \
		$(if $(SMG_BASE_URL),SMG_BASE_URL='$(SMG_BASE_URL)' )$(NPM) test

# --- Seed a load-test course + token for the load tests ---------------------------
# Needs a running, installed Moodle. Prints `export BASE_URL/TOKEN/QUIZID/
...
load-seed: clear
	...
	@echo ""
	@echo "Saved BASE_URL/TOKEN/QUIZID/CMID to $(LOAD_DIR)/.load-env"
...
jmeter: clear jmeter-setup
	...
	@if [ -z "$(TOKEN)" ] || [ -z "$(QUIZID)" ] || [ -z "$(CMID)" ]; then \
		echo "Missing required parameters. Usage:"; \
		echo "  make jmeter BASE_URL=<wwwroot> TOKEN=<token> QUIZID=<id> CMID=<id> \\"; \
		...
		echo "  or pass TOKEN/QUIZID/CMID yourself. See tests/load/README.md."; \
		exit 1; \
	fi
	cd $(LOAD_DIR) && $(JMETER) -n -t stackmathgame-read-endpoints.jmx \
		-Jbase_url='$(BASE_URL)' -Jtoken='$(TOKEN)' \
		-Jquizid='$(QUIZID)' -Jcmid='$(CMID)' -Jrevision='$(REVISION)' \
		-Jthreads='$(THREADS)' -Jrampup='$(RAMPUP)' -Jloops='$(LOOPS)' \
		-Jmaxduration='$(MAXDURATION)' \
		-l stackmathgame-load-results.jtl
	@echo ""
